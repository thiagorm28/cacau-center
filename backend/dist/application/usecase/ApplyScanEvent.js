...
const toResolution = (event) => {
    if (event.result === "unidentified")
        return { kind: "unidentified" };
    if (event.noteId === null || event.noteItemId === null)
        return { kind: "unidentified" };
    const kind = event.result === "exceeded" ? "exceeded" : "matched";
    return { kind, noteId: event.noteId, itemId: event.noteItemId };
};
let ApplyScanEvent = ApplyScanEvent_1 = class ApplyScanEvent {
    constructor(unitOfWork) {
        this.unitOfWork = unitOfWork;
        this.logger = new common_1.Logger(ApplyScanEvent_1.name);
    }
    async execute(input) {
        const applied = await this.unitOfWork.run((repositories) => this.applyWithin(repositories, input));
     ...
     */
    async applyWithin(repositories, input) {
    ...
    async plan(repositories, openNotes, input) {
        if (input.manualItemId !== undefined)
            return this.planManual(openNotes, input.manualItemId);
        if (input.markUnidentified === true) {
            return { resolution: { kind: "unidentified" }, result: "unidentified" };
        }
        const resolution = (0, shared_1.resolveScan)(openNotes.map((note) => note.toPendingNote()), input.scannedCode);
     ...
     */
    async planExceededOnCompleted(repositories, scannedCode) {
        const unidentified = {
        ...
        if (completedNotes.length === 0)
            return unidentified;
        const resolution = (0, shared_1.resolveScan)(completedNotes.map((note) => note.toPendingNote()), scannedCode);
    ...
    planManual(openNotes, manualItemId) {
        const note = openNotes.find((candidate) => candidate.findItem(manualItemId) !== undefined);
        if (note === undefined)
            throw new DomainErrors_1.NotFoundError("Item não encontrado nas notas em conferência");
        const item = note.findItem(manualItemId);
        if (!item.isPending()) {
            return {
     ...
     */
    async settle(repositories, planned, operatorId) {
        if (planned.result !== "matched" && planned.result !== "manual_matched")
            return planned;
        const { noteId, itemId } = planned.resolution;
    ...
    async confirmBox(repositories, planned, itemId, operatorId) {
        ...
        if (persisted === null || !persisted.isFullyConfirmed())
            return;
     ...
     */
    async closeCompleted(repositories, note, operatorId) {
...
ApplyScanEvent = ApplyScanEvent_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, common_1.Inject)("UnitOfWork")),
    __metadata("design:paramtypes", [Object])
], ApplyScanEvent);
exports.default = ApplyScanEvent;
//# sourceMappingURL=ApplyScanEvent.js.map